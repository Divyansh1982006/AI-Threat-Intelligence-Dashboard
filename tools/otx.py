...
def get_indicator(ioc_type, indicator_type, value):
    ...
    try:
        ...
        if response.status_code == 404:
            # A 404 here means OTX has no pulse data for this specific
            # indicator -- that's a meaningful "0 reports" result, not
            # a system failure. Treating it as an error would throw
            # away a legitimate (if unexciting) reputation signal.
            return {
                "status": "success",
                "ioc": value,
                "ioc_type": indicator_type,
